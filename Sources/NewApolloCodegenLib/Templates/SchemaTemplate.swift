...
struct SchemaTemplate: TemplateRenderer {
  ...
  private func protocolDefinition(prefix: String?, schemaName: String) -> TemplateString {
    TemplateString("""
      public protocol \(prefix ?? "")SelectionSet: NewApolloAPI.SelectionSet & NewApolloAPI.RootSelectionSet
      where Schema == \(schemaName).Schema {}

      public protocol \(prefix ?? "")InlineFragment: NewApolloAPI.SelectionSet & NewApolloAPI.InlineFragment
      where Schema == \(schemaName).Schema {}

      public protocol \(prefix ?? "")MutableSelectionSet: NewApolloAPI.MutableRootSelectionSet
      where Schema == \(schemaName).Schema {}

      public protocol \(prefix ?? "")MutableInlineFragment: NewApolloAPI.MutableSelectionSet & NewApolloAPI.InlineFragment
      where Schema == \(schemaName).Schema {}
      """
    )
  }
